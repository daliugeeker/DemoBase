 ...
 * @since 2018/9/12
 */
public class LeoPlayerView extends FrameLayout {
    ...
    private final LeoPlayerControlView controller;
    ...
    public LeoPlayerView(Context context) {
        this(context, null);
    }

    public LeoPlayerView(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public LeoPlayerView(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        if (attrs != null) {
            TypedArray a =
                context.getTheme().obtainStyledAttributes(attrs, R.styleable.PlayerView, 0, 0);
            try {
                ...
                keepContentOnPlayerReset =
                    a.getBoolean(R.styleable.PlayerView_keep_content_on_player_reset,
                        keepContentOnPlayerReset);
        ...
        LeoPlayerControlView customController = findViewById(R.id.exo_controller);
        ...
        if (customController != null) {
            ...
            this.controller = new LeoPlayerControlView(context, null, 0, attrs);
    ...
    public LeoPlayerControlView getVideoController() {
        return controller;
    }
     ...
     * @param LeoPlayerView The new view to attach to the player.
     */
    public static void switchTargetView(@NonNull Player player,
        @Nullable LeoPlayerView oldPlayerView, @Nullable LeoPlayerView LeoPlayerView) {
        if (oldPlayerView == LeoPlayerView) {
            return;
        }
        ...
        if (LeoPlayerView != null) {
            LeoPlayerView.setPlayer(player);
        }
     ...
     * {@link #switchTargetView(Player, LeoPlayerView, LeoPlayerView)} rather than this method. If you do
    ...
    @Override public boolean dispatchKeyEvent(KeyEvent event) {
        ...
        return isDpadWhenControlHidden || dispatchMediaKeyEvent(event) || super.dispatchKeyEvent(
            event);
    }
    ...
    private void maybeShowController(boolean isForced) {
        ...
        if (useController) {
            boolean wasShowingIndefinitely =
                controller.isVisible() && controller.getShowTimeoutMs() <= 0;
    ...
    private void hideArtwork() {
        if (artworkView != null) {
            artworkView.setImageResource(
                android.R.color.transparent); // Clears any bitmap reference.
    ...
    private void updateBuffering() {
        if (bufferingView != null) {
            boolean showBufferingSpinner = showBuffering
                && player != null
