 ...
 * @since 2018/9/12
 */
public class LeoPlayerControlView extends FrameLayout {

    static {
        ExoPlayerLibraryInfo.registerModule("goog.exo.ui");
    }

    private String TAG = DBLog.DB_TAG + "new_player_control";
    ...
    private com.google.android.exoplayer2.ui.PlayerControlView.VisibilityListener
        visibilityListener;
    ...
    public LeoPlayerControlView(Context context) {
        this(context, null);
    }

    public LeoPlayerControlView(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public LeoPlayerControlView(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    public LeoPlayerControlView(Context context, AttributeSet attrs, int defStyleAttr,
        ...
        if (playbackAttrs != null) {
            ...
            try {
                ...
                controllerLayoutId =
                    a.getResourceId(R.styleable.PlayerControlView_controller_layout_id,
                        controllerLayoutId);
                repeatToggleModes = getRepeatToggleModes(a, repeatToggleModes);
                showShuffleButton = a.getBoolean(R.styleable.PlayerControlView_show_shuffle_button,
                    showShuffleButton);
            } finally {
                a.recycle();
            }
        ...
        videoList.setOnClickListener(new OnClickListener() {
            @Override public void onClick(View v) {
                DBLog.d(TAG, "video list clicked");
                if (null != extraListener) {
                    extraListener.onListButtonClick();
                }
            }
        });

        View fullScreen = findViewById(R.id.exit_full_screen);
        fullScreen.setOnClickListener(new OnClickListener() {
            @Override public void onClick(View v) {
                DBLog.d(TAG, "full screen clicked");
                if (null != extraListener) {
                    extraListener.onFullScreenClick();
                }
            }
        });
        ...
        previous.setOnClickListener(new OnClickListener() {
            @Override public void onClick(View v) {
                DBLog.d(TAG, "previous clicked");
                if (null != extraListener) {
                    extraListener.onPrevious();
                }
            }
        });
        ...
        next.setOnClickListener(new OnClickListener() {
            @Override public void onClick(View v) {
                DBLog.d(TAG, "next clicked");
                if (null != extraListener) {
                    extraListener.onNext();
                }
            }
        });
    ...
    @SuppressWarnings("ResourceType") private static @RepeatModeUtil.RepeatToggleModes
     ...
     * @param extraPlayedAdGroups Whether each ad has been played, or {@code null} to show no extra
     * ad
     * markers.
     */
    ...
    public void setVisibilityListener(
        com.google.android.exoplayer2.ui.PlayerControlView.VisibilityListener listener) {
    ...
    public void setControlDispatcher(
        ...
        this.controlDispatcher =
            controlDispatcher == null ? new com.google.android.exoplayer2.DefaultControlDispatcher()
                : controlDispatcher;
    }
    ...
    private void updateTimeBarMode() {
        ...
        multiWindowTimeBar =
            showMultiWindowTimeBar && canShowMultiWindowTimeBar(player.getCurrentTimeline(),
                window);
    }
    ...
    private void updateProgress() {
        ...
        if (player != null) {
            ...
            if (timeBar != null) {
                ...
                System.arraycopy(extraAdGroupTimesMs, 0, adGroupTimesMs, adGroupCount,
                    extraAdGroupCount);
                System.arraycopy(extraPlayedAdGroups, 0, playedAdGroups, adGroupCount,
                    extraAdGroupCount);
            ...
            if (player.getPlayWhenReady() && playbackState == Player.STATE_READY) {
                ...
                if (playbackSpeed <= 0.1f) {
                    ...
                    long mediaTimeUpdatePeriodMs =
                        1000 / Math.max(1, Math.round(1 / playbackSpeed));
                    long mediaTimeDelayMs =
                        mediaTimeUpdatePeriodMs - (position % mediaTimeUpdatePeriodMs);
    ...
    private void previous() {
        ...
        if (previousWindowIndex != C.INDEX_UNSET && (player.getCurrentPosition()
            <= MAX_POSITION_FOR_SEEK_TO_PREVIOUS || (window.isDynamic && !window.isSeekable))) {
            ...
            if (player != null) {
                if (nextButton == view) {
                    ...
                    if (player.getPlaybackState() == Player.STATE_IDLE) {
                        ...
                        controlDispatcher.dispatchSeekTo(player, player.getCurrentWindowIndex(),
                            C.TIME_UNSET);
                    }
                    controlDispatcher.dispatchSetPlayWhenReady(player, true);
                    ...
                    controlDispatcher.dispatchSetRepeatMode(player,
                        RepeatModeUtil.getNextRepeatMode(player.getRepeatMode(),
                            repeatToggleModes));
                } else if (shuffleButton == view) {
                    controlDispatcher.dispatchSetShuffleModeEnabled(player,
    ...
    public void addExtraListener(ExtraListener listener) {
